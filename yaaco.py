...
class ACO(Problem):
    ...
    def __init__(self, ants, filename, rho=0.5, alpha=1.0, beta=2.0,
        ...
        # Initialize the Problem instance
        Problem.__init__(self, filename, euclidean, ptype=instance_type,
...
#            plt.ylim([min(self.y) - 50, max(self.y) + 50])
    ...
    def run(self):
        ...
        # 1. Initialize data
        #    This step was already done in the __init__ function

        # 2. Loop
#        self.net_construct_solutions()
...
#            c = ant.tour[step-1]  # Current node
...
#                    print(" Selected node: {0}".format(N))
                    C.append(N)
...
#            print(" Next arcs: {0}".format(arcs_connected))
...
#                    print(" Removing arc: {0} ({1})".format(arc,
#                          self.base_graph[arc]))
                    AA.remove(arc)
                else:
                    ...
                    if arc_nodes[0] in ant.tour and arc_nodes[1] in ant.tour:
                        ...
                        # current tree
                        AA.append(arc)
#                        print(" Adding arc: {0} ({1})".format(arc,
#                              self.base_graph[arc]))
...
if __name__ == "__main__":
    ...
    aco1 = ACO(m, file_name, rho, alpha, beta, nn_ants, max_iters,
               use_base_graph=True, instance_type=problem)
    best = aco1.run()
